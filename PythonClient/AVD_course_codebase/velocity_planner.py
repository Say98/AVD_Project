...
class VelocityPlanner:
    ...
    def compute_velocity_profile(self, path, desired_speed, ego_state, 
                                 ...
                                 lead_car_state, follow_lead_vehicle,obstacle_on_lane, emergency_stop):
        ...
        internal parameters of interest:
            ...
            emergency_stop: variable to fire an emergency stop
        returns:
        ...
        if emergency_stop:
            profile = self.decelerate_profile(path, start_speed, stop= float('inf'))
        ...
        # speed by the time we reach the time gap point.
            ...
            profile = self.follow_profile(path, start_speed, desired_speed, 
                                          lead_car_state)
        # if there's an obstacle on our trajectory, speed profile for reaching 0 m/s
    ...
    def decelerate_profile(self, path, start_speed,stop=0): 
        ...
        # time.
        if brake_distance + decel_distance + stop_line_buffer + stop > path_length:
            speeds = []
            vf = 0.0
